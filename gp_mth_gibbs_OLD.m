...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
acc_noise_all = 0; acc_f_all = 0; acc_Kf_all = 0; gidx = 1:50; 
for g = 1:opt.nGibbsIter
    ...
    %InvK    = inv(K);  % this can be done faster using partitioned inverses
    ...
    % Duplicate the inputs (one gp for each class)
    InvKc    = blkdiag(InvKc,InvKc);
    LogDetKc = 2*LogDetKc;
    fc       = [f(~rid); -f(~rid)];
    ...
    if opt.UseRMHMC
        ...
        fc_new       = fc_new(:,end); % just take the last sample (for consistency)
    end
    % test acceptance
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   
    % conditional prior
    %CondPrior_mur = (K(rid,~rid)/Kc)*f(~rid);
    ...
    %f_new(rid) = CondPrior_mur + CondPrior_Kr*alpha;
