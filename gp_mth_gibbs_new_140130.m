...
for g = 1:opt.nGibbsIter
    ...
    KrcInvKc = (K(rid,~rid)/Kc);
    mur      = KrcInvKc*f_new(~rid);
...
%     % Quantities based on Sr needed for HMC    
%     Sr_noise = Sr + Noise(rid,rid);
%     InvSr    = inv(Sr_noise);
%     L_Sr     = chol(Sr_noise)'; 
%     LogDetSr = 2*sum(log(diag(L_Sr)));
%     
%     fxargs_f = {Yc, InvSc, LogDetSc, muc, InvSr, LogDetSr, f(rid), KrcInvKc};
